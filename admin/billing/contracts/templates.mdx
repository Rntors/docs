...
* `{contract.expirationDate}` — deadline for signing the contract

##### Contract details

* `{contract.reference}` — unique reference assigned to the contract
* `{notes}` — notes added to the contract
...
You can continue numbering additional customer signatories sequentially, such as `{customer.signatory4.fullName}`, `{customer.signatory5.fullName}`, and beyond. The same numbering pattern applies to `email`, `phoneNumber`, `signature`, and `signature_date`.
...
You can continue numbering additional location signatories sequentially, such as `{location.signatory3.fullName}`, `{location.signatory4.fullName}`, and beyond. The same numbering pattern applies to `email`, `phoneNumber`, `signature`, and `signature_date`.
...
* `{subtotal}` — total undiscounted price for all items in the contract
* `{grandTotal}` — final total amount for the contract
* `{total}` — final contract total when placed outside the `{#items}` and `{/items}` loop

<Info>
The meaning of `{total}` depends on where it is placed:

* Inside the `{#items}` loop, it represents the undiscounted total for the current item.
* Outside the loop, it represents the final total for the contract.
