...
5. To add an item, click **Create item**.
   ...
   - Select a [**one-off product or service from the Product list**](#replace-product-list-link).
   - Or add a **Custom** item.

6. [**Manage the Payment due**](#replace-payment-due-date-link) date in the right-side panel.
...
2. Go to the **General > Billing details** tab to [**update your billing details**](#replace-location-billing-details-link).
...
3. [**Add or update the customer’s billing details**](#replace-customer-billing-details-link).
4. Issue the invoice. It will reflect the updated details.

### Apply a discount

The administrator or customer should apply a discount when creating a [**subscription**](#replace-create-subscription-link), [**booking**](#replace-create-booking-link), or another purchase, rather than adding a discount inside the invoice later. This ensures the discount is automatically reflected on the invoice. If needed:
...
1. In the customer’s profile, add the discount amount to [**Money balance**](#replace-money-balance-link).
...
2. In the **Credit notes** section, create [**a credit note**](#replace-create-credit-note-link).
...
1. [**Void this invoice**](#replace-void-invoice-link).
2. Update [**customer**](#replace-customer-billing-details-link) or [**location**](#replace-location-billing-details-link) billing details.
...
1. Open the invoice.
2. In the **Credit notes** section, create [**a credit note**](#replace-create-credit-note-link) for the full amount.
3. Choose **Customer balance** as a credit option.
4. Update [**customer**](#replace-customer-billing-details-link) or [**location**](#replace-location-billing-details-link) billing details.
...
1. [**Issue a credit note**](#replace-create-credit-note-link) for this invoice.
...
<AccordionGroup>
  ...
  <Accordion title="How do I add bookings/products to a customer's invoice?">
    ...
    - **Bookings** — to add a booking to an invoice, make a booking on the **Resources** page, select the customer as a booker, and [**choose the Invoice payment method**](#replace-invoice-payment-method-link).
