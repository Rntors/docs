...
- Purchases made with the [**Invoice payment method**](#replace-invoice-payment-method-link), if they are configured to be added to subscription invoices.
...
2. Add a **Custom** item or select one of your [**pre-created one-off services**](#replace-one-off-services-link).
...
1. [**Edit the customer's subscription**](#replace-edit-customer-subscription-link) directly in their profile.
2. Modify or remove subscription items.
3. See the **Upcoming invoice** immediately reflect the changes.

### Other invoice items

- Items added by [**Pay with invoice**](#replace-pay-with-invoice-link) can be removed, but not modified.
...
1. [**Create a coupon**](#replace-create-coupon-link) for **Subscription items** with the required discount amount or percentage.
2. Edit the customer's subscription and [**apply the coupon**](#replace-apply-coupon-link) to the relevant subscription item.
...
2. Adjust the [**Money balance**](#replace-money-balance-link).
...
- A set number of days before renewal if you enabled [**Advance invoicing**](#replace-advance-invoicing-link).
...
3. In the **Payments** section, [**record the customer's payment**](#replace-record-customer-payment-link).
...
1. Go to the customer's profile.
2. Press **Adjust balance** near the **Money balance** and [**add a payment**](#replace-add-payment-link).
...
2. [**Pause payment collection**](#replace-pause-payment-collection-link) in it.
3. Choose to generate their subscription invoice as a draft or immediately void it.
4. Optionally, set a [**Resume date**](#replace-resume-date-link).
...
<Tip>
  Consider applying a [**100% discount**](#replace-full-discount-link) or a [**full amount credit**](#replace-full-amount-credit-link) instead of completely deleting or voiding the upcoming invoice. This will generate the invoice with a 0 total while keeping a record of the adjustment.
</Tip>
...
3. Either:
   ...
   - [**Schedule an end date**](#replace-schedule-end-date-link), or
   - [**Cancel the subscription**](#replace-cancel-subscription-link) immediately.
...
<AccordionGroup>
  ...
  <Accordion title="Can I disable automatic charging or issuing of the upcoming invoice?">
    ...
    2. [**Disable Auto-pay**](#replace-disable-auto-pay-link) in the right-hand panel.
  ...
  <Accordion title="Can customers check their upcoming invoice?">
    Yes. Members and [**company managers**](#replace-company-managers-link) can check their upcoming invoice as follows:
  ...
  <Accordion title="What happens if an upcoming invoice has been issued but a customer makes more payments with 'Invoice'?">
    ...
    For example, if January's invoice is issued in December and the customer later makes additional purchases using the [**Invoice payment method**](#replace-invoice-payment-method-link), one more upcoming invoice will be generated and include only those new payments.
  </Accordion>
</AccordionGroup>
