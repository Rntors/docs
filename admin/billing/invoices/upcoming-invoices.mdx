...
- Purchases made with the [**Invoice payment method**](/admin/billing/payments/pay-with-invoice), if they are configured to be added to subscription invoices.
...
2. Add a **Custom** item or select one of your [**pre-created one-off services**](/guides/catalog-of-reusable-fees).
...
1. [**Edit the customer's subscription**](/admin/billing/subscriptions/edit-a-subscription) directly in their profile.
2. Modify or remove subscription items.
3. See the **Upcoming invoice** immediately reflect the changes.

### Other invoice items

- Items added by [**Pay with invoice**](/admin/billing/payments/pay-with-invoice) can be removed, but not modified.
...
1. [**Create a coupon**](/admin/billing/discounts/coupons-and-promo-codes) for **Subscription items** with the required discount amount or percentage.
2. Edit the customer's subscription and [**apply the coupon**](/admin/billing/discounts/apply-discounts-to-bookings-and-purchases) to the relevant subscription item.
...
2. Adjust the [**Money balance**](/admin/billing/invoices/customer-money-balance).
...
- A set number of days before renewal if you enabled [**Advance invoicing**](/admin/billing/subscriptions/advance-invoicing).
...
3. In the **Payments** section, [**record the customer's payment**](/admin/billing/invoices/record-invoice-payments).
...
1. Go to the customer's profile.
2. Press **Adjust balance** near the **Money balance** and [**add a payment**](/admin/billing/invoices/customer-money-balance).
...
2. [**Pause payment collection**](/admin/billing/subscriptions/control-invoice-issuance) in it.
3. Choose to generate their subscription invoice as a draft or immediately void it.
4. Optionally, set a [**Resume date**](/admin/billing/subscriptions/control-invoice-issuance).
...
<Tip>
  Consider applying a [**100% discount**](/admin/billing/discounts/coupons-and-promo-codes) or a [**full amount credit**](/admin/billing/invoices/customer-money-balance) instead of completely deleting or voiding the upcoming invoice. This will generate the invoice with a 0 total while keeping a record of the adjustment.
</Tip>
...
3. Either:
   ...
   - [**Schedule an end date**](/admin/billing/subscriptions/cancel-a-subscription), or
   - [**Cancel the subscription**](/admin/billing/subscriptions/cancel-a-subscription) immediately.
...
<AccordionGroup>
  ...
  <Accordion title="Can I disable automatic charging or issuing of the upcoming invoice?">
    ...
    2. [**Disable Auto-pay**](/admin/billing/subscriptions/control-invoice-auto-charge) in the right-hand panel.
  ...
  <Accordion title="Can customers check their upcoming invoice?">
    Yes. Members and [**company managers**](/admin/customer-management/companies/company-members-and-managers) can check their upcoming invoice as follows:
  ...
  <Accordion title="What happens if an upcoming invoice has been issued but a customer makes more payments with 'Invoice'?">
    ...
    For example, if January's invoice is issued in December and the customer later makes additional purchases using the [**Invoice payment method**](/admin/billing/payments/pay-with-invoice), one more upcoming invoice will be generated and include only those new payments.
  </Accordion>
</AccordionGroup>
